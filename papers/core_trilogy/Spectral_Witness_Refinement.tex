...
\fancyhead[R]{\small \textit{Spectral Witness Survival}}
\fancyfoot[C]{\thepage}
...
\texttt{where\_my\_hunches\_went.md} (narrative letter) $\cdot$ \texttt{operator\_constraints.md} (constraint list C1--C12) $\cdot$ \texttt{uniformity\_addendum.md} (conditional-uniformity full statement) $\cdot$ \texttt{admissibility\_note.md} (filter, two failures, coupling). Source framework: \emph{Colour from Gravity}.
