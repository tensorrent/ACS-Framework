...
re-solve~\eqref{eq:Ecap}.  That revision is carried out as an
as-implemented numerical sequel in~\cite{ribboncap2026}
(annulus vs conformal-modulus vs M\"obius BIE collocation, with
aspect-ratio scan and machine-readable results).  Under those
revisions, moderate geometric aspect ratios yield
$\alpha^{-1}=\mathcal{O}(1)$ rather than $\approx 137$; the figure
$\alpha^{-1}\approx 137.036$ remains a numerical output of the
annulus model at a tuned cutoff, not a geometry-unique invariant of
the framed unknot.
...
\bibitem{ribboncap2026}
Flag Condensate Collaboration,
\textit{M\"obius-Ribbon Capacitance: Annulus, Conformal Modulus, and BIE
Revisions of the $\alpha$ Estimate},
preprint (July 2026),
\texttt{mobius\_ribbon\_capacitance.tex} /
\texttt{papers/notes/Mobius\_Ribbon\_Capacitance.tex}.
